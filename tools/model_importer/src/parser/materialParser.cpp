...
namespace {
  ...
  void readMaterialTileAxisFromJson(V64M::TileParam &param, const json &tex)
  {
    if(tex.empty())return;
  ...
  void readMaterialFromJson(const V64M::Config &config, V64M::MaterialTexture &material, const json &tex, const fs::path &gltfPath)
  {
  ...
  V64M::ColorCombiner readCCFromJson(const json &cc)
  {
    V64M::ColorCombiner res{};
  ...
  namespace CC = V64M::CC;

  bool isCCUsingTexture(const V64M::ColorCombiner &cc)
  {
  ...
  bool isUsingShade(const V64M::ColorCombiner &cc)
  {
...
V64M::Material V64M::parseMaterial(const Config &config, const fs::path &gltfBasePath, cgltf_primitive *prim) {
