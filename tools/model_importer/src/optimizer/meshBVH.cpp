 ...
 * @param modelChunks
 */
std::vector<int16_t> V64M::createMeshBVH(const std::vector<ModelFlat> &models)
{
  ...
  {
    aabbs.emplace_back(
      BVec3(model.aabbMin[0], model.aabbMin[1], model.aabbMin[2]),
      BVec3(model.aabbMax[0], model.aabbMax[1], model.aabbMax[2])
    );
