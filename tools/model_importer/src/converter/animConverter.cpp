...
namespace {
  ...
  bool hasConstValue(const std::vector<V64M::Keyframe> &keyframes, bool isRotation) {
    if(!isRotation) {
  ...
  bool isEmptyChannel(V64M::AnimChannelMapping &channel)
  {
    if(hasConstValue(channel.keyframes, channel.isRotation())) {
      bool isIdentity = false;
      const auto &kf = channel.keyframes[0];
      switch(channel.targetType) {
        case V64M::AnimChannelTarget::TRANSLATION  : isIdentity = fabsf(kf.valScalar) < MIN_VALUE_DELTA; break;
        case V64M::AnimChannelTarget::ROTATION     : isIdentity = kf.valQuat.isIdentity(); break;
        case V64M::AnimChannelTarget::SCALE_UNIFORM:
        case V64M::AnimChannelTarget::SCALE        : isIdentity = kf.valScalar == 1.0f; break;
      }
   ...
   */
  void optimizeChannel(V64M::AnimChannelMapping &channel, float time) {
  ...
  void quantizeRotation(V64M::Keyframe &kf)
  {
...
void convertAnimation(V64M::Anim &anim, const std::unordered_map<std::string, const V64M::Bone*> &nodeMap)
{
  ...
  std::sort(anim.keyframes.begin(), anim.keyframes.end(), [](const V64M::Keyframe &a, const V64M::Keyframe &b) {
  ...
  for(auto &kf : anim.keyframes)
  {
    ...
    if(ch.targetType == V64M::AnimChannelTarget::ROTATION) {
      quantizeRotation(kf);
    } else {
      kf.valQuantSize = 1;
