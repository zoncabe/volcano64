...
inline const V64M::Keyframe& safeKf(const std::vector<V64M::Keyframe> &kfs, int idx) {
  if(idx < 0)return kfs[0];
  if(idx >= kfs.size())return kfs.back();
  return kfs[idx];
}

inline float calcMSE(const std::vector<V64M::Keyframe> &kfsNew, const std::vector<V64M::Keyframe> &kfsOrg, float timeStart, float timeEnd, bool isRotation) {
  ...
  for(float t=timeStart; t<timeEnd; t += timeStep)
  {
    ...
    const V64M::Keyframe &kfOrg = safeKf(kfsOrg, idxOrg);
    const V64M::Keyframe &kfOrgNext = safeKf(kfsOrg, idxOrg + 1);
    const V64M::Keyframe &kfNew = safeKf(kfsNew, idxNew);
    const V64M::Keyframe &kfNewNext = safeKf(kfsNew, idxNew + 1);
