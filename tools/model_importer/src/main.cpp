...
int main(int argc, char* argv[])
{
  ...
  if(args.checkArg("--help")) {
    printf("Usage: %s <gltf-file> <modelData-file> [--bvh] [--base-scale=64] [--ignore-materials] [--ignore-transforms] [--asset-path=assets] [--verbose]\n", argv[0]);
    printf("Params:\n");
  ...
  const std::string outPath = args.getFilenameArg(1);
  ...
  auto modelData = V64M::parseGLTF(gltfPath.c_str(), config);
  writeModel(config, modelData, outPath);
}
