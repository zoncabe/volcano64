...
void printBoneTree(const V64M::Bone &bone, int depth)
{
...
V64M::ModelData V64M::parseGLTF(const char *gltfPath, const V64M::Config &config)
{
  ModelData modelData{};
  ...
  for(int i=0; i<data->skins_count; ++i) {
    ...
    while((boneCount+neutralBoneCount) < skin.joints_count)
    {
      ...
      modelData.skeletons.push_back(armature);
    }
  }
  ...
  if(!modelData.skeletons.empty()) {
    ...
    for(const auto &skel : modelData.skeletons) {
  ...
  for(int i=0; i<data->animations_count; ++i) {
    ...
    modelData.animations.push_back(anim);
  }
  ...
  for(int i=0; i<data->nodes_count; ++i)
  {
    ...
    for(int j = 0; j < mesh->primitives_count; j++)
    {
      modelData.models.push_back({});
      auto &model = modelData.models.back();
      ...
      if(prim->material) {
        ...
        if(matName && matName[0] && modelData.materials.find(matName) == modelData.materials.end()) {
          modelData.materials[matName] = parseMaterial(config, gltfBasePath, prim);
        }
      ...
      std::vector<VertexOut> verticesT3D{};
      ...
      {
        auto &material = modelData.materials[model.materialName];
  ...
  std::sort(modelData.models.begin(), modelData.models.end(), [&modelData](const V64M::Model &a, const V64M::Model &b) {
    auto &matA = modelData.materials[a.materialName];
    auto &matB = modelData.materials[b.materialName];
